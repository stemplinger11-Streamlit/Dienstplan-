...
# ===== Twilio Balance (nur wenn nicht SAFE_MODE) =====
def get_twilio_balance():
    """Holt Twilio Account Balance"""
    ...
    try:
        ...
        return None, f"Fehler beim Abrufen der Balance: {str(e)}"
...
st.set_page_config(page_title="Dienstplan+ Cloud v5.0", page_icon="📅", layout="wide")
...
def _notify_admins_rebooking(old_user, new_user, slot, booking_date):
    ...
    message = format_template("admin_rebooking_notification", st.session_state.db,
                             OLD_USER=old_user["name"], NEW_USER=new_user["name"],
...
def _send_rebooking_confirmation(new_user, slot, booking_date):
    """Sendet Bestätigung an neuen Nutzer"""
...
# ===== UI: Auth =====
def ui_auth():
    st.markdown('<div class="main-header">🔐 Dienstplan+ Cloud v5.0</div>', unsafe_allow_html=True)
    ...
    for slot in WEEKLY_SLOTS:
        ...
        with col_action:
            if blocked:
                ...
                # Admin-Umbuchung
                if st.button("🔄 Umbuchen", key=f"rebook_{b['id']}"):
                ...
                with col1:
                    if st.button("✅ Umbuchen bestätigen", type="primary"):
                        ...
                        if st.session_state.db.rebook_to_user(st.session_state.rebook_booking_id, new_user_id):
                            ...
                            st.success("Schicht erfolgreich umgebucht!")
                            st.rerun()
    ...
    with admin_tabs[2]:
        ...
        with col1:
            ...
            if st.button("🔄 Balance aktualisieren"):
                ...
                if balance_data:
                    st.success(f"💳 Guthaben: {balance_data['balance']} {balance_data['currency']}")
                    st.session_state.twilio_balance = balance_data
                else:
                    st.error(f"❌ Fehler: {error}")
            
            if hasattr(st.session_state, "twilio_balance"):
                data = st.session_state.twilio_balance
                st.info(f"💳 Aktuelles Guthaben: {data['balance']} {data['currency']}")
        ...
        # Benutzerstatistiken "Wer-Wann-Wie oft"
        st.subheader("👥 Benutzerstatistiken")
                            ...
                            with zipfile.ZipFile(uploaded_file, 'r') as zip_file:
                                ...
                                if json_files:
                                    ...
                                    if success:
                                        ...
                                        # Optional: Auto-Reload nach 5 Sekunden
                                        st.balloons()
                                    else:
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button("📖 Handbuch öffnen", use_container_width=True):
            # Handbuch-Tab aktivieren (vereinfacht)
